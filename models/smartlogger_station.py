    ...
    def sync_fusionsolar_data(self):
        ...
        try:
            ...
            if stations_data.get('success'):
                ...
                _logger.error("FusionSolar API: Не вдалося отримати список станцій. Помилка: %s",
                              stations_data.get('failCode'))
                raise UserError(
                    _("Не вдалося отримати список станцій з FusionSolar API. Помилка: %s") % stations_data.get(
                        'failCode'))
    ...
    def _update_station_data(self, station_data, token, session, base_url):
        ...
        if not station:
            ...
            station = self.create({
                ...
                'status': 'active'  # Встановлюємо статус за замовчуванням
            })
        else:
        ...
        try:
            ...
            if kpi_response_json.get('success'):
                ...
                if kpi_list:
                    ...
                    # Если нет данных, устанавливаем статус error
                    station.write({'status': 'error'})
            else:
                _logger.error("FusionSolar API: Не вдалося отримати KPI для станції %s. Помилка: %s", station_code,
                              kpi_response_json.get('failCode'))
                # При ошибке API устанавливаем статус error
                station.write({'status': 'error'})
